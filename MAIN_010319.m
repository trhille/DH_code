...
%%
LGM_b_dot = 1;
precip_at_sl_LGM_vec = 0.2;
lapse_LGM_vec = [-0.06]./1500;
% lapse_LGM_vec = [-0.25, -0.3, -0.35]./1500;
for jj = 1:length(precip_at_sl_LGM_vec)
    precip_at_sl_LGM = precip_at_sl_LGM_vec(jj);
    
    for nn = 1:length(lapse_LGM_vec)
        lapse_LGM = lapse_LGM_vec(nn);


runname = ['precip_', strrep(num2str(precip_at_sl_LGM), '.', '_'),...
    'lapse_', strrep(num2str(lapse_LGM), '.', '_')];
disp('')
disp(runname)
disp('')
% - - - - - - - - -  - - - - - - - - - - - - -  
...
% RESET Hatherton from Darwin calculation
disp('  Resetting surface elevation for Hatherton based on Darwin values ... ')
S_in_global_Hatherton = h_w(iter, index_xpos_Hatherton_edges) + B_w(index_xpos_Hatherton_edges);
S_0_in2               = S_in_global_Hatherton;
S_at_GL2(iter)       = S_0_in2;
...
% Then, setup flux added at 17 km from Hatherton...
flux_add_P(index_xpos_Hatherton_P) = flux_edges_dyn_xt2(iter, 1) / W_P2(1);
[ flux_add_w, ...
  flux_add_e ] = get_edge_values_quadratic ...
                                  ( flux_add_P, x_P, x_w, x_e, dx_P, dx_w, dx_e );
...
% update bdot for next iteration
[ b_dot_edges(iter+1,:), b_dot_P(iter+1,:) ] = ...
    iterate_bdot( S_P(iter,:), precip_at_sl_LGM, lapse_LGM, x_P, x_edges );


[ b_dot_edges2(time+1,:), b_dot_P2(time+1,:) ] = ...
    iterate_bdot( S_P2(iter,:), precip_at_sl_LGM, lapse_LGM, x_P2, x_edges2 );

if iter>1
res_Darwin = sqrt((sum(S_P(iter,:) - S_P(iter-1,:))).^2);
res_Hat = sqrt((sum(S_P2(iter,:) - S_P2(iter-1,:))).^2);

res = sqrt(res_Darwin.^2 + res_Hat.^2);

disp(['Iteration ', num2str(iter), 'Residual = ', num2str(res)])
...
save(['TEST_DH/output/LGM_b_dot/', runname], 'S_P', 'S_P2', 'b_dot_edges',... 
    'b_dot_edges2', 'b_dot_P', 'b_dot_P2')
end  % if statement on steady_state

    end % end loop over lapse rate
end % end loop over precip at sea level
