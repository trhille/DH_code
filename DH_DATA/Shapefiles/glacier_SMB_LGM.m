function [width, SMB_LGM, distance_along_centerline] = glacier_SMB_LGM(RACMO_precip_path, flowline_path, buffer_path, nodes)
...
precip = RACMO_nc2mat(RACMO_precip_path);
SMB = 0.75.*precip.precip; %make the very simple assumption that the LGM SMB was 75% of modern accumulation (neglecting sublimation and runoff)
...
count = 1;
SMB_LGM = zeros(size(flowline,1), 1);
for jj = 1:size(flowline,1)
    ...
    SMB_LGM(jj) = sum(SMB_profile)./length(SMB_profile);
