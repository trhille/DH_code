...
if (deformation_only == 1)
    ...
    K_w_t = E_w .* flow_constant_t(1:end-1) .* (W_w.*scaling_w.*h_w_t) .* (h_w_t.^(n+1)) ...
            .* ( (dS_dx_w_t.^2).^( (n-1)/2 ) );
    K_e_t = E_e .* flow_constant_t(2:end) .* (W_e.*scaling_e.*h_e_t) .* (h_e_t.^(n+1)) ...
            .* ( (dS_dx_e_t.^2).^( (n-1)/2 ) );
        
elseif (sliding_only == 1)
    
    K_w_t = ( ( (fs_w .* ((rho_ice*g)^n .* h_w_t.^(n-1) .* (W_w.*scaling_w.*h_w_t))) ) ...
            .* ( (dS_dx_w_t.^2).^( (n-1)/2 ) ) );    

    K_e_t = ( ( (fs_e .* ((rho_ice*g)^n .* h_e_t.^(n-1) .* (W_e.*scaling_e.*h_e_t))) ) ...
            .* ( (dS_dx_e_t.^2).^( (n-1)/2 ) ) );           
    
elseif (deformation_plus_sliding == 1)
    
    K_w_t = ( (( E_w .* flow_constant(1:end-1) .* (W_w.*scaling_w.*h_w_t) .* (h_w_t.^(n+1))) ...
                + (fs_w .* ((rho_ice*g)^n .* h_w_t.^(n-1) .* (W_w.*scaling_w.*h_w_t))) ) ...
            .* ( (dS_dx_w_t.^2).^( (n-1)/2 ) ) );    

    K_e_t = ( (( E_e .* flow_constant(1:end-1) .* (W_e.*scaling_e.*h_e_t) .* (h_e_t.^(n+1))) ...
                + (fs_e .* ((rho_ice*g)^n .* h_e_t.^(n-1) .* (W_e.*scaling_e.*h_e_t))) ) ...
            .* ( (dS_dx_e_t.^2).^( (n-1)/2 ) ) );        
  ...
  flux_edges_dyn_t = [Q_w_t(1) Q_e_t] + [flux_add_w(1) flux_add_e];
