...
% Can't have negative coefficients so take (dS_dx_e.^2).^( (n-1)/2 )
    ...
    K_w   = ( (( E_w .* flow_constant(1:end-1) .* (W_w.*scaling_w.*h_w) .* (h_w.^(n+1))) ...
                + (fs_w .* ((rho_ice*g)^n .* h_w.^(n-1) .* (W_w.*scaling_w.*h_w))) ) ...
            .* ( (dS_dx_w.^2).^( (n-1)/2 ) ) );
        
    K_e   = ( (( E_e .* flow_constant(1:end-1) .* (W_e.*scaling_e.*h_e) .* (h_e.^(n+1))) ...
                + (fs_e .* ((rho_ice*g)^n .* h_e.^(n-1) .* (W_e.*scaling_e.*h_e))) ) ...
            .* ( (dS_dx_e.^2).^( (n-1)/2 ) ) );     
...
% check that this is the same as dynamic calculation: 
  ...
  Q_e_dyn = (Q_e_dyn_temp .* scaling_e) + flux_add_e;
