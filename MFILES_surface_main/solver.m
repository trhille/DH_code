...
% ===============================
    K_w   = E_w .* flow_constant(1:end-1) .* (W_w.*scaling_w.*h_w) .* (h_w.^(n+1)) ...
             .* ( (dS_dx_w.^2).^( (n-1)/2 ) );
    K_e   = E_e .* flow_constant(2:end) .* (W_e.*scaling_e.*h_e) .* (h_e.^(n+1)) ...
            .* ( (dS_dx_e.^2).^( (n-1)/2 ) );
...
% there cannot be any negative coefficients, so take dh_dx^2
...
% check that this is the same as dynamic calculation: 
  ...
  Q_e_dyn = (Q_e_dyn .* scaling_e) + flux_add_e;
...
% Prescribe surface elevation at the grounding line:
     rhs(1)    = rhs(1) + (2*(S_at_GL(2)) * theta * (1./W_P(1)) .* D_w_t(1));
