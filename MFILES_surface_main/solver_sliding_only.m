function [ S_P_t_guess, ...
           S_P_t, ...
           flux_edges_dyn_t ] = solver_sliding_only( flow_constant, flow_constant_t, ...
...
%   sliding_term = ( -sign(dS_dx) .* (fs .* (rho_ice * g * abs(dS_dx) ).^n ) .* h.^(n-1) );
   ...
   K_w   = ( ( (fs_w .* ((rho_ice*g)^n .* h_w.^(n-1) .* (W_w.*scaling_w.*h_w) )) ) ...
           .* ( (dS_dx_w.^2).^( (n-1)/2 ) ) );
    
    K_e   = ( ( (fs_e .* ((rho_ice*g)^n .* h_e.^(n-1) .* (W_e.*scaling_e.*h_e))) ) ...
            .* ( (dS_dx_e.^2).^( (n-1)/2 ) ) );     
